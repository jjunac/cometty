...
- `cargo test` — 199 unit tests in `grid`/`term`/`input`/`app::geometry`/`scrollbar`/`selection`/`theme`/`renderer` (incl. `renderer::blocks`); runs instantly, no services needed.
